...
class LinearQuantized(nn.Linear):
    ...
    def forward(self, x):
        weight = ste_function.apply(self.weight, self.init_weight.clone().detach().to(device))
        bias = ste_function.apply(self.bias, self.init_bias.clone().detach().to(device))
...
class LinearQuantized_granularity_channel(nn.Linear):
    ...
    def forward(self, x):
        weight = ste_function_granularity_channel.apply(self.weight, self.init_weight.clone().detach().to(device))
        bias = ste_function.apply(self.bias, self.init_bias.clone().detach().to(device))
        return F.linear(x, weight, bias)
...
class Conv2dQuantized_granularity_kernel(nn.Conv2d):
    ...
    def forward(self, x):
        weight = ste_function_granularity_kernel.apply(self.weight, self.init_weight.clone().detach().to(device))
        bias = ste_function.apply(self.bias, self.init_bias.clone().detach().to(device))
        return F.conv2d(x, weight, bias, self.stride, self.padding, self.dilation, self.groups)
